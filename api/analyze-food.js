export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `На фото — блюдо и референс (${`ВНИМАТЕЛЬНО посмотри на фото и определи ЧТО РЕАЛЬНО ТАМ ВИДНО.
На фото есть блюдо/продукт и референс для масштаба (${reference === 'карта' ? 'банковская карта' : reference === 'ложка' ? 'ложка' : 'стакан'}).
